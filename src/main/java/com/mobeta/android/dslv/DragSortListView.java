...
public class DragSortListView extends ListView {
    ...
    public void setSlideShuffleSpeed(float val) {
        mSlideRegionFrac = Math.max(0.0f, Math.min(1.0f, 1.0f - val));
    }

    public void setCollapsedHeight(int val) {
        mItemHeightCollapsed = Math.max(1, val);
    }
    ...
    private class AdapterWrapper extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView != null) {
                ...
                if (child.getParent()!=v) {
					// shouldn't get here if user is reusing convertViews properly
					//Log.d("mobeta", "sanity check removing the view...");
                    v.removeAllViews();
    ...
    private void dropFloatView() {
		...
		final int numHeaders = getHeaderViewsCount();
        int from = mSrcPos - numHeaders;
        int to =  mFloatPos - numHeaders;;
        boolean doit = mDropListener != null && mFloatPos >= 0 && mFloatPos < getCount();
		...
		if (doit) mDropListener.drop(from, to);
    }
	...
	public boolean ListViewIntercepted = false;
    ...
    private void adjustItem(int position, View v, boolean invalidChildHeight) {
        ...
        if(((DragSortItemView)v).getChildAt(0)==null)
        	height=1;
        
        if (height != lp.height) {
			//Log.d("mobeta", "set height="+height);
			lp.height = height;
