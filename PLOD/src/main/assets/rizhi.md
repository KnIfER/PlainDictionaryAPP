...
- DoDo群号 ： 37860 。开发者会优先查看此群中的发言与评论，整理用户需求与反馈，但不一定回复。毕竟比起打开就立即滴滴嘟嘟的聊天软件，打开dodo群不过是在浏览器中打开一个标签页，是轻而易举的事，开发者隔三岔五就会看看。
...
### 7.新增PDF阅读功能
- 正式支持打开PDF文件，支持高亮标注功能（需要有文字，暂不支持自由sticky note）。
