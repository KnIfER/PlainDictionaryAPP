...
# v8.4
- 文本竖排--新增词典级别设置：可开启文本竖排，以古法阅古诗。
- 长按链接--修复长按链接弹出菜单：可选择/复制链接文本、在本页打开、弹窗打开等。
- 点击翻译--修复可长按按钮：长按返回按钮、标题栏、收藏按钮、发音按钮触发更多功能。
- 资源加载--新增词典级别设置：设置是否加载词典同名css、是否检测.0.txt、是否允许从同目录加载文件资源。
- 外观--调整网页版外观：调整词典标题栏外观。夜间模式显示原色（前景色、背景色）。
- 多页面--优化展开折叠：待优化。
- 列表--新增返回动画：从内容页面返回词条列表时，为当前列表行播放缩放动画。
