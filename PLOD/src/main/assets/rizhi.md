# v6.5
### 1.笔记系统
- 新增笔记系统，可创建下划线/高亮标注（可定制颜色），并且可用括号/气泡/注释的形式为标注添加文本附注（前两者可定制文本大小与颜色，可选气泡颜色，气泡笔记可选择是否将笔记直接写入气泡中。注释笔记类似于zhihu/wiki网站的脚注系统，但也可选择将上标放入气泡中，使之更加明显）。创建后可修改。
- 笔记列表分全部笔记、当前词典笔记、当前页面笔记，可按照创建时间、词典名称、文本段落及其组合进行排序。
- 解释页面的文本选择菜单添加“高亮”与“笔记”两项。长按“高亮”添加下划线标注，长按“笔记”修改已选标注或笔记。长按笔记列表可进行复制、分享或删除。

### 2.点击翻译、快速翻译栏
- 优化点击翻译，更准确地定位字符。可为中文开启大内存取词模式（默认开启，取词组而非单字，需要分析整段文本）。
- 谷歌翻译退出，普通网络无法将之用作在线词典。
- 彩云小译失效，无法直接翻译其他词典的内容，但仍可用作在线词典。
- 预置添加百度、彩云、必应、有道四种在线翻译站点，合称“百彩必有”。
- 并优化翻译型在线词典的调用方式为：浏览选项-开启悬浮小按钮后，选择文本会在屏幕右下角显示悬浮按钮，在此按钮往左上滑，可开启翻译型词典列表，快速翻译选中的文本。默认使用“点击翻译”界面，长按可用其他方式打开。

### 3.点选文本
- 解释页面，“翻译当前页面”对话框的第二条可打开点选模式，包括段落点选、单词点选，无需长按即可选取文本。点选后，在原处再次点击，应该可调出原生的文本选择菜单。
...
# v5.10
- 嵌入在线应用社区“DoDo”与“贴吧”，可通过词典管理器添加，或在侧滑抽屉“关于”页面中查看。
- DoDo超级群号 ： 37860 

# v5.9
- 新功能 ：在底部工具栏->宫格菜单中实现了分享按钮。可分享**单词**或**网址**，点击分享列表的第一个格子以进行设置。
