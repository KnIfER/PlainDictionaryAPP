# 前言
- 在线应用社区“DoDo”/“贴吧”，可通过词典管理器添加，或在侧滑抽屉“关于”页面中查看。
- DoDo群号 ： 37860 。开发者会优先查看此群中的发言与评论，整理用户需求与反馈，但不一定回复。毕竟比起滴滴嘟嘟的聊天软件，访问dodo这种公开群不过是在浏览器中打开一个新的标签页，轻而易举。
...
### 3.优化“点击翻译”、新增“快速翻译栏”
- “点击翻译”能够更准确地定位字符。可为中文开启“大内存取词”模式（默认开启，取词组而非单字，需要分析整段文本）。
- 在“点击翻译”界面，点击标题栏可预览词条上下文，上下滑动双列列表可翻页。
- 谷歌翻译退出，普通网络无法将之用作在线词典，无法在“点击翻译”中使用。彩云小译失效，无法直接翻译页面内容，但仍可用作在线词典，配合划词使用。
- 预置添加百度/彩云/必应/有道四种在线翻译站点，合称“百彩必有”。
- 并优化翻译型在线词典的调用方式为：“浏览选项”-“文本工具”按钮后，选择文本会在屏幕右下角显示悬浮按钮，在此按钮往左上滑，可开启翻译型词典列表——“快速翻译栏”，可快速翻译选中的文本。默认使用“点击翻译”界面，长按可用其他方式打开。

### 4.新增“点选文本”功能，可快速划词翻译
- 解释页面-顶栏菜单按钮，打开“翻译当前页面”对话框，可切换点选模式，包括“段落点选”、“单词点选”，无需长按即可选取文本。点选后，在原处再次点击，应该可调出“系统的文本选择菜单”。
- “段落点选”开启了“自动缩小选取范围”的功能(暂时强制)，点选段落后，再次点击同一段落，可点选其中的单词，如需呼出“系统的文本选择菜单”，可长按须臾后松开，或点击段落选区的开头/末尾。

### 5.优化“文本工具”双列对话框
- 双列列表可上下滑动翻页，不再需要点击切换按钮。
- 长按列表中最后的几个项目，可自定义分享目标的名称。
- 可自定义分享目标共八个。长按列表以定制各个字段，内置字段名设有_chooser/_flag/_share，长按“保存”按钮可恢复默认值，更改后测试以查看作用。其中“_chooser”字段与直接打开目标还是打开系统选择器相关(如值c创建选择器、值q总是查询而忽略系统默认值，建议组合为c/q)，“_flag”字段与系统的意图标志有关（建议填写n，代表打开新的任务），“_share”字段设置以某种类型打开内置的分享列表，值可为none/send/search，分别代表不生效/发送文本/搜索文本，不可组合。

### 6.其他界面优化
- 使沉浸式滚动可伸缩页面长度，底栏不遮挡页面底部。
- 新增“页边距”功能，可设置解释页面的左右边距。（“多页面设置”与“词典设置”，默认3%）
- 为两个“双列列表”实现念珠式滑动翻页，有助于参禅入定。
- 长按图片、词典标题栏、页面链接、词条列表等显示新的弹出式菜单。

### 7. Bug Fixes
- 解决JS传递书籍id参数精度不足导致跳转失败等问题（偶发严重）。
- 解决某些弹出界面在切换横竖屏后，意外留下不透明popup窗口，导致触控失效（偶发严重）。
