# 公告
- 建议用电脑收藏[DoDo群网址](https://www.imdodo.com/channel/157568/889287) 。我会不时地查看、分享新功能！


# v7.6
- 微调旧多页面翻页阈值（防止与点击事件冲突）、新增拖动进度条的提示效果。
- 微调网页导航栏（长按标题栏打开），增加更多按钮：TTS暂停、上下翻页（无动画）。
- 微调点击翻译左下角按钮，改为弹出更多功能菜单：切换上一词典、全文朗读、选择文本时有快捷翻译菜单，功能与快捷翻译栏一致、页内搜索、分享页面、钉住点击翻译对话框。
- 微调词典和分组对话框的弹出方式，改为默认从底部弹出。
- 微调全文朗读，可选择朗读语言、开启自动判断朗读语言、开启句中分中英文。
- TTS控制器添加快捷翻译栏，可发送文本等。
- 长按快捷翻译栏可弹出点击翻译界面（不搜索）。
- 底部工具栏 宫格菜单 新增第二页有：。
- 优化词典管理器界面菜单，改长按为双列点按。
- 词典管理器搜索框常显，显示搜索结果列表，显示搜索结果数目。
- 修复词典管理器批量自定义词典设置。

# v7.5
- 长按发音按钮得TTS控制器，可设置TTS引擎。
- 更新安卓依赖库。
