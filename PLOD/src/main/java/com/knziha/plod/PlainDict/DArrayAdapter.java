	...
	public void createNewDatabase(String name) {
		try {
			if (testDBV2) {
				...
				notebooksV2.add(new MyPair<>(name, a.prepareHistoryCon().newFavFolder(name)));
			} else {
				...
				notebooks.add(new MyPair<>(name, new LexicalDBHelper(a, a.opt,name,false)));
			}
		} catch (Exception e) {
			a.showT("创建失败！");
		}
